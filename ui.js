$(async function() {
  ...
  // add story form event handler.

  $submitForm.on("submit", async function(evt) {
    evt.preventDefault(); //because it's a form
    ...
    // post API and get back story object
    ...
    // add the newly created story to the DOM
    const $li = $(`
      <li id="${storyObject.storyId}" class="id-${storyObject.storyId}">
        <span class="star"><i class="far fa-star"></i></span>
        <a class="article-link" href="${url}" target="a_blank">${title}</a>
  ...
  // Add and remove favorites event handler
  ...
  $(".articles-container").on("click", ".star", async function(evt) {
    if (currentUser) {
      ...
      // if favorited remove and change class to empty star
      if ($evtTarget.hasClass("fas")) {
        await currentUser.removeFavorite(storyId);
        $evtTarget.closest("i").toggleClass("fas far");
      } else {
        // change to not- empty fas star
        await currentUser.addFavorite(storyId);
        $evtTarget.closest("i").toggleClass("far fas");
      }
    }
  });
  ...
  function showNavForLoggedInUser() {
    $navLogin.hide();
    $userProfile.hide();
    // add submit/favorites/mystories
